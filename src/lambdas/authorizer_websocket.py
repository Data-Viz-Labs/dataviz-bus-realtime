"""
WebSocket API Custom Authorizer for Madrid Bus Simulator
Validates API key from query parameter against Secrets Manager
Validates presence of group_name query parameter
Returns IAM policy allowing or denying the WebSocket connection
"""
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Custom authorizer for WebSocket API Gateway.
    Validates API key from query parameter.
    Validates group_name from query parameter.
    Returns IAM policy allowing or denying the connection.
    ...
    """
    print(f"WebSocket Authorizer event: {json.dumps(event)}")
    ...
    # Validate group_name parameter presence
    if not group_name:
        print("Missing group_name parameter")
        raise Exception('Unauthorized: Missing group_name parameter')
    ...
    print(f"WebSocket authorization successful for group: {group_name}")
...
def generate_policy(principal_id: str, effect: str, resource: str, group_name: str) -> Dict[str, Any]:
    ...
    to support caching across different routes.
    
    Args:
        principal_id: Principal identifier
        effect: Allow or Deny
        resource: Method ARN (used to extract API ARN)
        group_name: Group name from query parameter
        
    Returns:
        IAM policy document
    """
    # Extract the API ARN and create a wildcard resource
    # resource format: arn:aws:execute-api:region:account:api-id/stage/route
